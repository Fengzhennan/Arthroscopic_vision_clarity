...
from backbone import generator, discriminator, TinyRRDBNet, UNetDiscriminatorSN, TinyUNetDiscriminatorSN, TinyUNetDiscriminatorSN_Label
...
def train():
    ...
    netD_B = TinyUNetDiscriminatorSN_Label().to(device_0)

    # gpu 1
    netG_B2A = TinyRRDBNet().to(device_1)
    netD_A = TinyUNetDiscriminatorSN_Label().to(device_1)
    ...
    # loss_A2B
    loss_gan_A2B = torch.nn.MSELoss().to(device_0)      # loss_GAN = torch.nn.BCEWithLogitsLoss().to(device_0)
    ...
    # loss_B2A
    loss_gan_B2A = torch.nn.MSELoss().to(device_1)       # loss_GAN = torch.nn.BCEWithLogitsLoss().to(device_1)
    ...
    label_real_0 = torch.ones([1], dtype=torch.float,
                            requires_grad=False).to(device_0)
    label_real_1 = torch.ones([1], dtype=torch.float,
                              requires_grad=False).to(device_1)
    label_fake_0 = torch.zeros([1], dtype=torch.float,
                             requires_grad=False).to(device_0)
    label_fake_1 = torch.zeros([1], dtype=torch.float,
                               requires_grad=False).to(device_1)
        ...
        for i, batch in enumerate(dataloader):
            ...
            with torch.cuda.amp.autocast():
                ...
                if (epoch % 10 == 0 or epoch == 200) and i == 10:
                    ...
                    hemorrhage_estimation = wandb.Table(data=data, columns=columns)

                    wandb.log({"Real-time supervision": hemorrhage_estimation})
                ...
                print("Tv_loss:{:.6f}".format(tv_loss_B2A_))
                print("\n" + "——————————————————————" * 8 + "%s" % nowtime)
                ...
                wandb.log({"Total_loss_net_G_B2A :": loss_G_B2A,
                           ...
                           "tv_loss_B2A": tv_loss_B2A_, "iter": step,
                           })
...
if __name__ == '__main__':
    ...
    sweep_id = wandb.sweep(sweep=sweep_config, project='hemorrhage imitation')
